...
def draw_heap(heap_root):
    ...
    nx.draw(heap, pos=pos, labels=labels, arrows=False, node_size=2000, node_color=colors)
    plt.show()

# Створення бінарної купи
heap_root = Node(0)
heap_root.left = Node(1)
heap_root.right = Node(4)
heap_root.left.left = Node(5)
heap_root.left.right = Node(3)
heap_root.right.left = Node(10)
